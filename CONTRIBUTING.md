# Contributing to upmex

First off, thank you for considering contributing to upmex! It's people like you that make upmex such a great tool.
...
```bash
# Clone your fork
git clone https://github.com/your-username/upmex.git
cd upmex
...
```bash
# 1. Bump the version in pyproject.toml and src/upmex/__init__.py, add a CHANGELOG entry, merge it
...
Confirm the upload with the version-specific endpoint, `https://pypi.org/pypi/upmex/X.Y.Z/json`.
The aggregate `https://pypi.org/pypi/upmex/json` is cached and can lag a few minutes behind,
which looks like a failed upload when it is not.

## Community

You can chat with the community on:
- [GitHub Discussions](https://github.com/SemClone/upmex/discussions)
