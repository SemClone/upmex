...
### Fixed
- **A licence declared only by URL is identified again.** A POM names its licence as prose plus a URL, and the prose (`The Apache Software License, Version 2.0`) is not something any detector should classify on its own, so the URL carries the answer. upmex read that URL by handing osslili the string `License: <url>` and letting its tag patterns recognise the address. osslili 1.8.0 stopped recognising addresses, and because the dependency is floored rather than pinned, every Java package declaring its licence by URL silently began reporting the prose instead of an SPDX id.
  The URL is now resolved by what it names or by what it serves, never by its spelling. Addresses whose meaning is fixed (`apache.org/licenses/LICENSE-2.0`, `opensource.org/licenses/MIT`, and the like) are read from a table with no request at all. Anything else is fetched in registry mode and identified from the text that comes back, which also covers URLs no table lists.
  Fetching is gated on registry mode, since it reaches the network, and a declared URL is untrusted input: only `http` and `https` are fetched, redirects are followed by hand so every hop is checked rather than only the address the package named, and a host resolving anywhere off the public internet — loopback, private, link-local, carrier-grade NAT, a cloud instance's metadata address — is refused. The response is read in chunks against a size cap that holds after decompression, so a small compressed body cannot expand into a large one.
  Only evidence about a document as a whole — its hash, an SPDX tag it carries, or its text matching a licence closely — may name the licence a URL declares. gnu.org's LGPL-2.1 page quotes the GPL-2.0 at length, so a phrase match over it reports `GPL-2.0-only` with more confidence than the `LGPL-2.1-only` the URL actually names, and reporting the wrong licence is worse than reporting none.
- A licence file named `licenses.txt` is read as one. osslili classifies the plural as documentation where `LICENSE.txt` is a licence file, and upmex dropped the package's own licence over the `s`.

### Changed
- A README carrying an entire licence is now read as declaring it during a directory scan, not only when that file is scanned on its own. This was previously given up on: osslili scored a short window around the word "license", so a bare mention scored the same as a document carrying the whole licence and both had to be refused. It scores the whole file from 1.8.0, which separates them. A README that only mentions a licence — crediting a bundled dependency, say — is still refused.
- `osslili>=1.8.0` is the new floor, for the whole-file scoring the rule above depends on.
...
### Fixed
- **Maven identity fields no longer read from the wrong element**: a POM nests `<version>` and `<url>` inside `<parent>`, `<licenses>`, `<scm>`, `<dependencies>` and `<build>`, and every lookup in the Maven path used a descendant search, so values belonging to something else were reported as the artifact's own. Each is now read from the project element, with legitimate Maven inheritance kept as an explicit fallback (closes #93, #94, #96):
  - `version` came from `<parent>`, so any module under an independently versioned parent was misreported — `commons-lang3-3.12.0` extracted as version `52`, its `commons-parent` version, and the PURL followed. A module that declares no version of its own still inherits the parent's.
  - `homepage` fell through to a `<license><url>` when a POM declared no project `<url>` — `gson-2.10.1` reported the Apache License text as its homepage.
  - `dependencies.runtime` included `<dependencyManagement>` pins and build-plugin dependencies — `commons-lang3-3.12.0` listed checkstyle, spotbugs, bnd and a JUnit BOM as runtime dependencies of the shipped jar.
  - `description`, `scm`, `licenses`, `developers` and `contributors` had the same exposure and are now scoped the same way.
- **Maven PURLs are spec-compliant**: the groupId is the PURL namespace and keeps its dots, but it was written with slashes — `pkg:maven/com/squareup/okhttp3/okhttp` instead of `pkg:maven/com.squareup.okhttp3/okhttp`. A PURL is the key every other tool joins on, so the malformed form silently failed to match SBOM, vulnerability, ClearlyDefined and PurlDB data, and could not be parsed back into a groupId. **This changes the `purl` field for every Maven, Jar and Gradle package** (closes #95). Two related corrections:
  - A `maven` PURL requires its namespace, so an artifact whose groupId could not be determined now has `purl: null` instead of a namespace-less `pkg:maven/<name>` that no consumer could match.
  - Maven coordinates were only split into namespace and name for the `maven` package type, while `jar` and `gradle` map to the same PURL type. Registry and API lookups for those built `pkg:maven/<groupId>:<artifactId>` and silently matched nothing — reachable as soon as a POM-less jar gained coordinates. All coordinate splitting now runs through one helper, `split_namespace()`.
- **Dependencies from build plugins and inactive profiles are excluded**: whether a profile was active is not knowable from a published archive, since it depends on build properties, the JDK and the OS, so profile-conditional dependencies are left out rather than attributed to the artifact unconditionally.
...
### Changed
- Moving to official repository and simplifying name
- Renamed project from `semantic-copycat-upmex` to `upmex`
- Updated repository URLs to point to `https://github.com/SemClone/upmex`
...
### Changed
- Renamed project from `package-metadata-extractor` to `upmex`
