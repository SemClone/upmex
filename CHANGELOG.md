...
### Fixed
- **A licence declared only by a URL is identified again.** A POM commonly declares its licence as prose plus a URL, and the prose (`The Apache Software License, Version 2.0`) is not classifiable on its own, so the URL carries the answer. upmex resolved it by passing osslili the string `License: <url>` and relying on its tag patterns to recognise the address. osslili 1.8.0 stopped recognising addresses; because the dependency is floored rather than pinned, every Java package declaring its licence by URL began reporting the prose in place of an SPDX id, with nothing to announce the change.
  URLs are now resolved by upmex. Canonical addresses (`apache.org/licenses/LICENSE-2.0`, `opensource.org/licenses/MIT` and similar) are read from a table, with no request. Any other URL is fetched in registry mode and identified from the text it serves, which also covers addresses the table does not list.
  Fetching happens only in registry mode. A declared URL is untrusted input, so `http` and `https` are the only schemes fetched; redirects are followed one hop at a time and every hop is checked; a host resolving off the public internet (loopback, private, link-local, carrier-grade NAT, cloud metadata addresses) is refused; and the body is read in chunks against a size cap applied after decompression.
  Only whole-document evidence may name the licence: an exact hash, an SPDX tag, or text matching a known licence at 0.95 or above. A phrase found somewhere inside a page is not enough, because a page can quote a licence it does not carry. gnu.org's LGPL-2.1 page quotes the GPL-2.0 at length, and a phrase match over it scores GPL-2.0-only above the LGPL-2.1-only the URL names.
- A licence file named `licenses.txt` is read as one. osslili classifies the plural as documentation where `LICENSE.txt` is a licence file, so the package's own licence was dropped.

### Changed
- A README containing an entire licence is read as declaring it during a directory scan, not only when that file is scanned on its own. osslili previously scored a short window around the word "license", which left a bare mention indistinguishable from the full text, so both were refused. It scores the whole file from 1.8.0. A README that only mentions a licence, such as one crediting a bundled dependency, is still refused.
- `osslili>=1.8.0` is the new floor, required by the whole-file scoring above.
